...
<head>
    ...
    <title>Vice Chancellor Dashboard</title>
</head>
<body>
<?php
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action_project"])) {
    ...
    $checkStatusSQL = "SELECT * FROM vc_accepted WHERE project_id = '$projectId'";
    ...
    if ($statusResult && mysqli_num_rows($statusResult) > 0) {
        ...
        // Fetch details for the selected project from reg_accepted
        $fetchProjectDetailsSQL = "SELECT * FROM reg_accepted WHERE project_id = '$projectId'";
        ...
        if ($projectDetailsResult && mysqli_num_rows($projectDetailsResult) > 0) {
            ...
            if ($actionType === "accept") {
                // Insert details into 'vc_accepted' table (using INSERT IGNORE)
                $insertAcceptedProjectSQL = "INSERT IGNORE INTO vc_accepted 
                                             (project_id, research_title, department, rating, starting_date, num_installments, 
                                             account_number, ifsc_code, micr_code, progress_report, progress_report_date, 
                                             total_amount, remaining_balance, total_spent_amount)
                                             VALUES 
                                             ('{$projectDetails['project_id']}', '{$projectDetails['research_title']}', 
                                             '{$projectDetails['department']}', '{$projectDetails['rating']}',
...
// Display project action status
if (isset($_GET["action_status"])) {
    $actionStatus = $_GET["action_status"];
    if ($actionStatus === "accepted") {
        echo "<p class='status-message accepted-message'>This project has already been accepted.</p>";
    } elseif ($actionStatus === "rejected") {
        echo "<p class='status-message rejected-message'>This project has already been rejected.</p>";
    }
}

// Fetch data from 'reg_accepted' table
$fetchProjectsSQL = "SELECT * FROM reg_accepted";
$result = mysqli_query($conn, $fetchProjectsSQL);

if ($result && mysqli_num_rows($result) > 0) {
    echo "<h2 style='margin-bottom: 20px;'>Welcome, Vice Chancellor!</h2>";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $checkAcceptedSQL = "SELECT * FROM vc_accepted WHERE project_id = '$projectId'";
