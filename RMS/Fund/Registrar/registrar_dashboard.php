...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action_project"])) {
    ...
    $checkStatusSQL = "SELECT * FROM reg_accepted WHERE project_id = '$projectId'";
    ...
    if ($statusResult && mysqli_num_rows($statusResult) > 0) {
        ...
        $fetchProjectDetailsSQL = "SELECT * FROM fd_accepted WHERE project_id = '$projectId'";
        ...
        if ($projectDetailsResult && mysqli_num_rows($projectDetailsResult) > 0) {
            ...
            if ($actionType === "accept") {
                // Insert details into 'reg_accepted' table (using INSERT IGNORE)
                $insertAcceptedProjectSQL = "INSERT IGNORE INTO reg_accepted 
                    ...
                    VALUES (
                        '{$projectDetails['project_id']}', '{$projectDetails['rating']}', 
...
// Fetch data from 'fd_accepted' table
$fetchProjectsSQL = "SELECT * FROM fd_accepted";
...
if ($result && mysqli_num_rows($result) > 0) {
    echo "<h2 style='margin-bottom: 20px;'>Welcome, Registrar!</h2>";
    echo "<h3>Accepted Projects Overview:</h3>";
    echo "<table border='1'>
            <tr>
                <th>Project ID</th>
                <th>Rating</th>
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $checkAcceptedSQL = "SELECT * FROM reg_accepted WHERE project_id = '$projectId'";
    ...
    echo "<p>No accepted projects found.</p>";
}
